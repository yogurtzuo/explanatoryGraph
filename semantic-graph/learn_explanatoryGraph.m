...
load(['./mat/cifar_10_lenet/',netname,'/roughCNN.mat'],'batch_f','batch_f_flip','conf','stat_all');
model=initializeModel(conf,batch_f{1});
imgNum=length(batch_f);
layerNum=length(conf.convnet.targetLayers);
display(conf.convnet.targetLayers)
...
for layerID=layerNum:-1:1
    ...
    modelName = '';
    for zpx=1:length(conf.convnet.targetLayers)
        modelName = [modelName, '_', int2str(conf.convnet.targetLayers(zpx))];
    end
    modelName = ['model', modelName, '.mat'];
    disp(modelName);
    save(['./mat/cifar_10_lenet/', netname, '/', modelName],'model');
 
    fprintf('save ./mat/model.mat after layer %d\n', layerID)
end
end
